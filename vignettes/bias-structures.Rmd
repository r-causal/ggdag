...
In addition to helping identify causal effects, the consistency of directed acyclic graphs (DAGs) are very useful in thinking about different forms of bias. Often, many seeminingly unrelated types of bias take the same form in a DAG. Methodological issues in a study often reduce to a problem of 1) not adequetely blocking a back-door path or 2) selecting on some variable that turns out to be a collider. 
...
But if you think about it, this DAG is incorrect. Smoking doesn't cause coffee drinking; something else, say a prediliction towards addictive substances, causes a person to both smoke and drink coffee. Let's say that we had a psychometric tool that accurately measures addictive behavior, so we can control for it. Now our DAG looks like this:
...
Which one is the confounder, addictive behavior or smoking? In fact, either will do, but only one or the other needs to be controlled for to block the path:

```{r}
ggdag_adjustment_set(coffee_dag, text = FALSE, use_labels = "label")
```

So, we should be concerned with *confounding* rather than *confounders*. 

# Colliders, M-bias, and butterfly bias

Collder-stratification bias is a major culprit in forms of systematic bias. Controlling for a collider--a node where two or more arrow heads meet--induces an association between its parents, through which confounding can flow:
...
Here the example is fairly simple and easy to deal with: m is a child of x and y and just shouldn't be adjusted for. It can get more complicated, though, when m is something that seems like it is a confounder or when it represents a variable that contributes to whether or not a person enters the study. Often this takes the form of M-shaped bias, or M-bias. Let's consider an example from [Modern Epidemiology](https://books.google.com/books/about/Modern_Epidemiology.html?id=Z3vjT9ALxHUC): the association between education and diabetes. Let's assume there's no association between the two. When we are putting together our analysis, we ask: should we adjust for the participant's mother's history of diabetes? It's linked to education via income and to participant's diabates status via genetic risk, so it looks like this:
...
surgical_dag <- dagify(ready_tool ~ readiness,
                       ...
                       outcome = "pain_change",
                       latent = "readiness",
                       labels = c(ready_tool = "Measured Readiness", 
                                  pain_change = "Change in Pain", 
                                  readiness = "Underlying Readiness", 
                                  pain = "Baseline Pain", 
                                  surgery = "Surgical Status"),
                       coords = coords2list(coords)) %>% 
  control_for("surgery")

ggdag_adjust(surgical_dag, text = FALSE, use_labels = "label")
```

We can't unstratify by surgical status, so our best bet is to block the back-door path opened by stratifying on it. We can't adjust for the latent variable (and, if we're measure it really well, it's one in the same with the tool), so all we can do is control for baseline pain. Note that, technicaly, surgical status is part of the adjustment set:

```{r}
ggdag_adjustment_set(surgical_dag, text = FALSE, use_labels = "label")
```

What about if a variable is both a collider and a cause of the exposure and outcome (a classical confounder)? This is an extension of M-bias sometimes called butterfly bias or bowtie bias. The variable is a part of a path that it's blocking as a collider *and* a back-door path that is confounding the relationship between x and y. What to do?

```{r}
ggdag_butterfly_bias(edge_type = "diagonal")
```

The strategy is basically the same as above; we need to control for m to block the back-door path, but that opens up a relationship between a and b, so we need to black that path, too. 
...
Measurement error the degree to which we mismeasure a variable. Measurement error can lead to bias in a number of ways. If the error is dependent on the exposure or the outcome, (e.g. we measure the exposure with less accuracy for a group without a disease than with it), it is called differential measurement error. If the error has nothing to do with the exposure or outcome, it's called non-differential measurement error. Under most conditions, non-differential error will bias the estimate of effect towards the null. In this way, it's at least predictable, but for small effects or inadequetely powered studies, it can make a true effect dissapear. If there is error in both the exposure and outcome, the errors themselves can also be associated, opening a back-door path between the exposure and outcome. 

Let's consider an example with non-differential error for the outcome and differential error for the exposure. One common situation this can occur is in cases of recall bias. Let's say we want to know if taking multivitamins in childhood helps protect against bladder cancer later in life.
...
Let's also consider how bias in loss-to-follow-up arises. In a randomized clinical trial or cohort study, the main threat of selection bias is not through who enters the study (although that may affect generalizability) but who leaves it. If loss-to-follow-up is associated with the exposure or outcome, the relationship between the two may be bias. Let's consider a trial where we are testing a new HIV drug and its effect on CD4 white blood cell count. If the treatment causes symptoms, participants may leave the trial. Similarly, there may be those whose HIV is getting worse and thus more symptomatic. If we only have information on people who stay in the study, we are stratfying by follow-up status:
...
dagify(follow_up ~ symptoms,
  ...
  ggdag_adjust("follow_up", text = FALSE, use_labels = "label")
```

# Resources

Many, if not all, forms of systematic bias can be drawn as DAGs. Here are a few sources of interesting examples:
