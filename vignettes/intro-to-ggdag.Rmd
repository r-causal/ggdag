...
Currently, `ggdag` supports directed (`x ~ y`) and bidirected (`a ~~ b`) relationships
...
There are also geoms based on those in `ggrepel` for inserting text and labels, and a special geom caled `geom_dag_collider_edges()` that highlights any biasing pathways opened by adjusting for collider nodes. See the [vignette introducing DAGs](intro-to-dags.html) for more info.
