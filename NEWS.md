...
* `geom_dag_edges()` draws with the edge engine and edge routing the options ask for, so a plot you assemble yourself gets the edges the packaged plots draw. It built the two ggraph edge layers whatever `edge_engine` and `edge_route` said, so a figure written as `ggplot(dag, aes_dag()) + geom_dag_edges() + geom_dag_point()` was drawn identically under `edge_route = "spline"` and `"orthogonal"`, and said nothing about it. It now takes `edge_engine`, `edge_route`, and `edge_route_options` arguments, defaulting to the matching options, and under the ggarrow engine builds the same routed layer `geom_dag()` builds. `arrow_directed` and `arrow_bidirected` are `grid::arrow()` specifications that no ggarrow layer can draw, so a call that supplies one under the ggarrow engine warns and takes its ornaments from the `arrow_head` and `arrow_fins` options instead; the formal defaults are left alone. `geom_dag_edges_link()`, `geom_dag_edges_arc()`, `geom_dag_edges_diagonal()`, and `geom_dag_edges_fan()` are ggraph layers whatever the engine option says, and each now warns once that a routing it was asked for is dropped. The packaged plots report an ignored routing once for the whole plot, as they did, and `ggdag_classic()` and `ggdag_paths_fan()` join the plotters that report it.

* A parameter written in a label geom wrapper now reaches the label layer. `geom_dag()` sets `size` and `col` on the label layer it assembles, so a wrapper of the shape `function(...) geom_dag_label_auto(..., size = 4.6)`, the natural way to restyle labels across a document, wrote those names a second time. ggplot2 kept the value `geom_dag()` had written and discarded the wrapper's, warning about a duplicated aesthetic for `size` and about an unknown parameter for `col`, so the layer was drawn exactly as it would have been without the wrapper. The label geoms now keep the last value written for a name, which is the wrapper's, and no warning is emitted. `geom_dag_label_auto()`, `geom_dag_text_auto()`, `geom_dag_label_repel()`, and `geom_dag_text_repel()` all behave this way, as do `geom_dag_label_repel2()` and `geom_dag_text_repel2()`, which pass their arguments on. A parameter `geom_dag()` does not set still reaches the layer as it did. Writing your value before the dots, as in `function(...) geom_dag_label_auto(size = 4.6, ...)`, still gives way to the value `geom_dag()` writes.

* `ggdag_adjustment_set()` draws the paths that adjusting for a collider activates. Such a path is an association rather than an edge of the DAG, and it was either dropped from the picture under the ggraph engine or drawn as an ordinary bidirected arc under the ggarrow engine. It is now drawn as a dashed curve under either engine, as `ggdag_adjust()` has always drawn it, and it is kept out of the ordinary directed and bidirected edge layers. The new `collider_lines` argument, `TRUE` by default, suppresses the curves when set to `FALSE`. A DAG with no such path is drawn as before.
...
* The automatic label geoms draw something when nothing maps `label`. A plot that added `geom_dag_label_auto()` or `geom_dag_text_auto()` with no mapping of its own drew no labels at all, and said nothing about it. Such a layer now falls back in order: the labels the DAG carries, and the node names when it carries none, so a DAG with labels draws them and a DAG without draws its node names. A `label` column of nothing but blanks and missing values counts as no labels. A `label` mapping you write, on the layer or on the plot, still wins over the fallback, and the `label` mapping `geom_dag(use_labels = TRUE)` generates on a DAG carrying no labels is still the silent no-op it was. `geom_dag_text()` and `geom_dag_label()` name their nodes as they always have. A missing DAG aesthetic is reported as well: the automatic and repelled label geoms now check the aesthetics they read before reading them, so a plot whose mapping leaves out `x` says which aesthetic is missing rather than failing on an internal subscript or on a `label` that resolved to the base function of that name.
