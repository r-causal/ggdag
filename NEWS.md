...
* New global option `auto_curve` (default `FALSE`). When it is `TRUE`, `tidy_dagitty()` checks every straight directed edge against the other nodes of the DAG and, where a node sits on or near an edge's straight path, writes a per-edge curvature that routes the edge around it, bowing away from the side where the intruding nodes sit and escalating the bow until the drawn arc clears every node. Curvature you set yourself, through `curved()`, `curve_edge()`, or DAGitty control points, is never overridden, and bidirected edges keep the arc their edge layer draws them with. The routing is deterministic, so the same DAG always comes out the same. The written curvature is drawn by the ggarrow edge engine: set `ggdag_options_set(auto_curve = TRUE, edge_engine = "ggarrow")` to see the routed edges.

* New `geom_dag_routed_arrows()` draws directed edges that detour around the nodes on their paths. Each blocked edge takes the shortest path through a visibility graph built over the tangent points of the obstacle circles, expanded to 1.5 node radii so the drawn path keeps a clearance margin of half a node radius, and the corners are smoothed with Chaikin corner cutting, so an edge weaves cleanly between nodes on opposite sides of its chord where a single arc could not. Unblocked edges stay straight, bidirected edges are drawn as arcs, curvature you set through `curved()` or `curve_edge()` is never rerouted, and the routing is deterministic. The automatic label geoms treat the routed paths as the edge obstacles, so labels avoid the drawn detours. The companion global option `auto_route` (default `FALSE`) swaps the routed layer into the packaged edge rendering: set `ggdag_options_set(auto_route = TRUE, edge_engine = "ggarrow")` to have `geom_dag()` and `ggdag()` route their directed edges.
