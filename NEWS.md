...
* The `curvature` option now reaches the ggraph edge layers. `geom_dag(edge_type = "arc")`, the quick plotting functions that build their own ggraph edge layers, and the bidirected arc of `geom_dag_edges()` all bend by the amount `curvature` resolves to, as the ggarrow engine already did. Arcs under ggraph were fixed at 0.5 and the bidirected arc at 0.3, so the two engines now bow the same edge by the same amount, and the default bend of `edge_type = "arc"` is 0.3 rather than 0.5, which draws shallower arcs. `geom_dag_edges_arc()` called directly keeps its own default of 0.5, and `ggdag_options_set(curvature = 0.5)` restores the old bow everywhere.
