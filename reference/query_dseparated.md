...
Test whether sets of variables are d-separated in a DAG given a
conditioning set.
...
#> # A tibble: 1 × 7
#>   from_set from      to_set to        conditioning_set conditioned_on dseparated
#>   <chr>    <list>    <chr>  <list>    <chr>            <list>         <lgl>     
#> 1 {x}      <chr [1]> {z}    <chr [1]> {}               <chr [0]>      FALSE     
query_dseparated(dag, from = "x", to = "z", conditioned_on = "w")
#> # A tibble: 1 × 7
#>   from_set from      to_set to        conditioning_set conditioned_on dseparated
#>   <chr>    <list>    <chr>  <list>    <chr>            <list>         <lgl>     
#> 1 {x}      <chr [1]> {z}    <chr [1]> {w}              <chr [1]>      TRUE      
```
