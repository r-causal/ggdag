...
test_that("deprecated argument handlers work when argument is not present", {
  ...
  test_deprecated <- function(new_arg = NULL, old_arg = deprecated()) {
    if (is_present(old_arg)) {
      deprecate_warn("1.0.0", "test_deprecated(old_arg)", "test_deprecated(new_arg)")
      new_arg <- old_arg
    }
    new_arg
  }
...
test_that("if_not_tidy_daggity with pathological inputs", {
  ...
  expect_error(if_not_tidy_daggity("not a dag"), "Input must be a dagitty object")
...
test_that("update_dag works correctly", {
  ...
  expect_equal(names(dagitty::edges(pull_dag(updated))), names(dagitty::edges(new_dag)))
})