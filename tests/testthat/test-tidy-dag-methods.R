test_that("fortify methods work correctly", {
  ...
  expect_true(all(c("name", "x", "y", "to", "xend", "yend") %in% names(fortified_tidy)))
  
  # Test fortify.dagitty
  fortified_dag <- fortify(dag)
  expect_s3_class(fortified_dag, "data.frame")
  expect_true(all(c("name", "x", "y", "to", "xend", "yend") %in% names(fortified_dag)))
})
...
test_that("print method works correctly", {
  ...
  tidy_dag4 <- activate_collider_paths(tidy_dag4, from = "x", to = "y", adjust_for = "m")
